...
void FaceAdjMesh::InitializeBBox(double min[], double max[])
{
...
void FaceAdjMesh::Initialize(double *p0, double *p1, double *p2)
{
	dTriFace* boundtri;
...
dTriFace *FaceAdjMesh::GetFace(int idx)
{
...
int FaceAdjMesh::NewVertex(double x, double y)
{
...
int FaceAdjMesh::NewTriFace(dTriFace *&temp)
{
	temp = new dTriFace;
...
void FaceAdjMesh::DeleteTriFace( int idx)
{
...
void FaceAdjMesh::Splite(int inIdx, int pntIdx )
{
...
void FaceAdjMesh::SwapPush(dSwapEdge *se, std::queue<dSwapEdge> &swapedgelist)
{
...
void FaceAdjMesh::Swap(dSwapEdge *se)
{
...
bool FaceAdjMesh::InTriangle(dTriFace *tri,double x, double y)
{
	double pc[2];
	pc[0] = x;
	pc[1] = y;
...
int FaceAdjMesh::LocatePoint(double x, double y)
{
	double xy[2] = {x,y};
	dDynaArrayW dylist;
...
void FaceAdjMesh::InsertPoint(double x, double y)
{
...
bool FaceAdjMesh::InTriCircle(dTriFace *tri, double x, double y)
{
	double plist[3][2];
	for ( int i=0; i<3; ++i ) {
...
void FaceAdjMesh::DumpMesh( const char *filename )
{
...
void FaceAdjMesh::DumpMeshFem( const char *filename )
{
	FILE *fp = fopen(filename,"w");
	srand(NULL);
...
void FaceAdjMesh::PushBBox(int fid, dTriFace *tri)
{
	//mVerts[tri->vIds[0]];
	dPoint2d pMin;
	dPoint2d pMax;
